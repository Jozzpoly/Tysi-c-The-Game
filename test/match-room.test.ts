...
function nextMessage(socket: WebSocket): Promise<any> {
  return new Promise((resolve, reject) => {
    ...
    socket.addEventListener('message', (event) => {
      ...
      try { resolve(JSON.parse(event.data as string)); } catch { resolve(event.data); }
    }, { once: true });
  });
}
...
async function createTrio(stub: MatchStub): Promise<{ create: CreateRoomAccepted; joins: [JoinRoomAccepted, JoinRoomAccepted]; tokens: [string, string, string] }> {
  ...
  expect([join1.seat, join2.seat]).toEqual([1, 2]);
...
describe('MatchRoom Durable Object lifecycle and authority', () => {
  it('creates an accountless trio lobby and atomically fills human seats without exposing credentials', async () => {
    ...
    expect((await stub.getPublicRoom())?.seats).toEqual(['human', 'human', 'human']);
    expect(await stub.joinRoom()).toMatchObject({ ok: false, reason: 'ROOM_FULL' });
    expect(JSON.stringify(await stub.getPublicRoom())).not.toContain('ts1_');
  });
  ...
  it('preserves hidden information across authenticated trio sessions', async () => {
    ...
    for (const seat of [0, 1, 2] as const) {
      const serialized = JSON.stringify(projections[seat]);
      for (const other of [0, 1, 2] as const) {
        if (other === seat) continue;
        for (const card of projections[other].observation.ownHand) expect(serialized).not.toContain(card);
      }
    }
  });
  ...
  it('runs bot-owned seats inside authority until the next human decision', async () => {
    ...
    const result = await stub.submitCommand(created.token, envelope('solo:human:1', initial, initial.legalCommands[0]));
  ...
  it('canonicalizes semantic retries, suppresses feedback replay and rejects command-id reuse', async () => {
    ...
    expect((await stub.submitCommand(tokens[2], firstEnvelope)).ok).toBe(true);

    const retry = await stub.submitCommand(tokens[2], { ...firstEnvelope, command: { seat: pass.seat, type: 'pass' } });
    ...
    if (!reused.ok) expect(reused.reason).toBe('CLIENT_COMMAND_ID_REUSED');
  });

  it('binds authorization to credential seat and serializes same-revision races', async () => {
  ...
  it('moves an existing authenticated lobby WebSocket into a started duo room', async () => {
    ...
    expect(host.first).toMatchObject({ type: 'lobby', seat: 0 });
  ...
  it('rejects invalid WebSocket credentials and cross-origin upgrades', async () => {
    const stub = env.MATCH_ROOM.getByName('ws-rejection-room');
    const { tokens } = await createTrio(stub);
  ...
  it('keeps authenticated per-seat WebSockets safe and usable across eviction', async () => {
    ...
    for (const card of opened2.first.projection.observation.ownHand) expect(JSON.stringify(opened0.first.projection)).not.toContain(card);
