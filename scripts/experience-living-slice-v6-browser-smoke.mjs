...
async function runSentence(session,{sentence,mobile,label}){
  ...
  await waitFor(`${label}: intent`,async()=> (await snapshot(session)).sentencePhase==='intent');
  snap=await snapshot(session);expectedOpacity(sentence,'intent',snap);
  ...
  await waitFor(`${label}: pending`,async()=> (await snapshot(session)).sentencePhase==='pending');
  snap=await snapshot(session);expectedOpacity(sentence,'pending',snap);
  assert(snap.phase==='pending'&&snap.hand.includes('AH'),`${label}: pending truth wrong`);
  await screenshot(session,`${label}-pending`);

  await waitFor(`${label}: resolving`,async()=> (await snapshot(session)).sentencePhase==='resolving');
  snap=await snapshot(session);expectedOpacity(sentence,'resolving',snap);
  ...
  await waitFor(`${label}: settled`,async()=> (await snapshot(session)).sentencePhase==='settled');
  snap=await snapshot(session);expectedOpacity(sentence,'settled',snap);
  ...
  return {
    sentence,
    intentThread:snap.overlays.intentThread.opacity,
    capturedValue:snap.capturedValue,
...
async function runReject(session,{mobile,label}){
  ...
  await end(session,mobile,target);
  await waitFor(`${label}: reject pending`,async()=> (await snapshot(session)).sentencePhase==='pending');
...
try{
  ...
  console.log('experience living slice v6 browser smoke: PASS');
  console.log(JSON.stringify({desktop,mobile},null,2));
}catch(error){
  console.error('experience living slice v6 browser smoke: FAIL');console.error(error);
