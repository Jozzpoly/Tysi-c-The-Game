...
assert(report.version === 1, `unexpected bench version ${report.version}`);
...
for (const row of [...report.baseline, ...report.densitySweep]) {
  for (const key of ['maxStepW', 'inactiveDriftW', 'jitterTotalVariationW', 'returnErrorW', 'totalDisturbanceW']) {
  ...
  assert(row.returnErrorW <= 1e-8, `${row.model}/${row.n}/${row.spacingW}: cancel failed to restore topology (${row.returnErrorW})`);
}
...
if (h0 && h1 && h2) {
  ...
  assert(h1.jitterTotalVariationW < h0.jitterTotalVariationW * 0.25,
...
console.log(`H2 max step=${h2.maxStepW.toFixed(4)}W, inactive drift=${h2.inactiveDriftW.toFixed(4)}W, jitter TV=${h2.jitterTotalVariationW.toFixed(4)}W`);
console.log('Reminder: this validates mechanical field properties only; it does not validate workspace cognition, feel, attention, or product preference.');
