...
async function inspect(session) {
  return execute(session, `
    ...
    return {
      ...
      dealerMarkers: [...document.querySelectorAll('.score small')].map((node) => node.textContent?.trim() ?? ''),
      scores: [...document.querySelectorAll('.score strong')].map((node) => node.textContent?.trim() ?? ''),
...
async function runEligible(label, width, height, mobile) {
  ...
  try {
    ...
    const initialDealer = option.dealerMarkers.findIndex((value) => value.includes('rozdaje'));
    ...
    await waitFor(`${label}: redeal`, async () => (await inspect(session)).heading === 'Licytacja');
    const redealt = await inspect(session);
    assertViewport(`${label}: redealt`, redealt, width);
    if (redealt.handCards !== 7) throw new Error(`${label}: redeal expected 7 cards, got ${redealt.handCards}`);
    if (redealt.dealerMarkers.findIndex((value) => value.includes('rozdaje')) !== initialDealer) throw new Error(`${label}: redeal rotated dealer`);
    ...
    return { option, continued, redealt };
  } finally {
