...
export function assertCoreInvariants(state: MatchState): void {
  ...
  const bombCounts = state.bombsUsed ?? [0, 0, 0];
  ...
  if (hand.phase === 'redeal-option') {
    if (hand.fourNinesSeat === null || hand.fourNinesSeat === undefined) throw new Error('redeal-option phase requires an eligible seat');
  ...
  } else if (hand.fourNinesSeat !== null && hand.fourNinesSeat !== undefined) {
  ...
  if (hand.completion?.kind === 'bomb') {
    ...
    if (hand.trickIndex !== 0 || captured.length !== 0 || pending.length !== 0) throw new Error('bombed hand must end before trick play');
    if (bombCounts[hand.completion.seat] !== hand.completion.bombNumber) throw new Error('bomb completion count must match match bomb counter');
  }

  if (hand.completion?.kind === 'played' && hand.phase !== 'complete') throw new Error('played completion requires complete hand phase');
  ...
  if (hand.lastCompletedTrick) {
    ...
    if (!hand.lastCompletedTrick.plays.some((play) => play.seat === hand.lastCompletedTrick?.winner)) throw new Error('completed trick winner must be one of its players');
  ...
  if (hand.phase === 'exchange' && hand.declarer !== null && hand.hands[hand.declarer].length !== 10) throw new Error('declarer must hold 10 cards before exchange');
