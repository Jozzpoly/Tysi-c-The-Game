...
export function advanceTactilePointer(
  state: TactilePointerState,
  input: AdvanceTactilePointerInput,
): TactilePointerState {
  ...
  const phase: TactileGesturePhase = commitReady
    ? 'accepted'
    : throwIntent
      ? 'table'
      : moved
...
export function classifyTactileRelease(
  state: TactilePointerState,
  options: { cancelled: boolean; canCommit: boolean },
): TactileReleaseOutcome {
