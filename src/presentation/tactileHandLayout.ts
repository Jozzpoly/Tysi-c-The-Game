...
export function estimateHandStep(centers: readonly number[]): number {
  const positive = centers.slice(1).map((center, index) => center - centers[index]).filter((distance) => Number.isFinite(distance) && distance > 0.5);
  return median(positive);
}

export function fractionalInsertionPosition(heldCenterX: number, centers: readonly number[]): number {
...
export function stabilizeInsertionTarget(input: { position: number; previousTarget: number; slotCount: number; hysteresis?: number }): number {
  ...
  if (candidate > previous) return input.position >= previous + 0.5 + margin ? candidate : previous;
  return input.position <= previous - 0.5 - margin ? candidate : previous;
}

export function computeHandInsertionPreview(input: { sourceIndex: number; heldCenterX: number; centers: readonly number[] }): HandInsertionPreview {
  const { sourceIndex, centers } = input;
  if (centers.length === 0) return { sourceIndex: 0, position: 0, targetIndex: 0, stepPx: 0, shiftsPx: [] };
  ...
  const shiftsPx = centers.map((_, index) => {
    ...
    if (position > safeSource && index > safeSource) return signedShift(stepPx, position - (index - 1), -1);
    if (position < safeSource && index < safeSource) return signedShift(stepPx, (index + 1) - position, 1);
    return 0;
  });
  return {
